...
async function testWebInterface() {
    ...
    try {
        ...
        console.log('\n2. Test /api/agent/status przez proxy...');
        const statusResponse = await axios.get(`${WEB_URL}/api/agent/status`);
        console.log('✓ Status:', statusResponse.data);
        
        // Test execute przez proxy
        console.log('\n3. Test /api/agent/execute przez proxy...');
        const executeResponse = await axios.post(`${WEB_URL}/api/agent/execute`, {
            command: 'echo Test'
        });
...
// Uruchom testy
console.log('Upewnij się, że zarówno Agent jak i Web Interface są uruchomione!');
console.log('Agent: http://localhost:8000');
